...
int main(int argc, char *argv[])
{
   ...
   float64 thresholdLevel = -20.00;    /* (dBm or dBm / Hz) */
   ...
   if (IQPowerEdgeEnabled)
   {
      RFmxCheckWarn(RFmxSpecAn_CfgIQPowerEdgeTrigger(instrumentHandle, "", "0", IQPowerEdgeLevel,
         RFMXSPECAN_VAL_IQ_POWER_EDGE_RISING_SLOPE,
         triggerDelay,
         RFMXSPECAN_VAL_TRIGGER_MINIMUM_QUIET_TIME_MODE_MANUAL,
         minQuietTime, RFMXSPECAN_VAL_TRUE));
   }
   else
   {
      RFmxCheckWarn(RFmxSpecAn_DisableTrigger(instrumentHandle, ""));
   }
   ...
   RFmxCheckWarn(RFmxSpecAn_TXPFetchMeasurement(instrumentHandle, "", timeout, &averageMeanPower, &peakToAveragePower,
