...
' 21. Select DPD measurement.Configure the reference waveform 
' 22. Set power of configured reference signal at the input of the DUT.                                    
' Select and configure the Decomposed Vector Rotation model to estimate the predistotor.
' Configure DPD measurement mode to 'Extract Only'.
' Set the measurement sample rate and the measurement interval to use for analysis.
' 23. Configure IDPD final optimal predistorted waveform as target waveform for DPD measurement.
' 24. Initiate DPD measurement and then configure Apply Digital Predistortion to remove the effects of memory and
' nonlinearity introduced by the DUT.
' 25. Fetch DVR model and NMSE results.
' 26. Abort RFSG generation and write a new Predistorted Waveform.
' Set Waveform Runtime Scaling to the desired Pre-filter Gain.
' Set the sample rate computed from Predistorted Waveform.
' Set the final PAPR to the sum of the actual PAPR and the Power Offset as computed from Predistorted Waveform.
' Set the Signal Bandwidth.
' Initiate RFSG generation using the script that was selected earlier
' 27. Select and configure AMPM measurement in RFmx after DPD measurement is complete.
' AMPM measurement is used to measure the AM-AM and AM-PM response of the DUT.
' 28. Initiate and fetch AMPM results.
' 29. Close RFmx session.
' 30. Close RFSG session. 
...
Public Class RFmxSpecAnDvrDpd
    ...
    Private referenceClockSource As RfsgFrequencyReferenceSource =
    ...
    Private dutAverageInputPower As Double = -20              ' dBm 
    Private selectedPorts As String = ""
    Private centerFrequency As Double = 1000000000.0                  ' Hz 
    Private referenceLevel As Double = 0.00                   ' dBm 
    Private rfsaExternalAttenuation As Double = 0.00          ' dB 
    Private rfsgExternalAttenuation As Double = 0.00          ' dB 
    Private preFilterGain As Double = -1.5                    ' dB 
    ...
    Private sampleRateMode As RFmxSpecAnMXIdpdMeasurementSampleRateMode =
        RFmxSpecAnMXIdpdMeasurementSampleRateMode.ReferenceWaveform
    Private sampleRate As Double = 120000000.0                      ' S/s 
    Private idleDurationPresent As RFmxSpecAnMXIdpdReferenceWaveformIdleDurationPresent =
        RFmxSpecAnMXIdpdReferenceWaveformIdleDurationPresent.False
    ...
    Private gainExpansion As Double = 3.0                    ' dB 
    ...
    Private startTime As Double = 0.00                        ' seconds 
    Private stopTime As Double = 0.0001                       ' seconds 
    ...
    Private targetGain As Double = 20                         ' dB 
    Private gain As Double = 0

    Private signalBandwidth As Double = 20000000.0                  ' Hz 
    Private autoLevelMeasurementInterval As Double = 0.0001   ' seconds 
    Private autoLevelReferenceLevel As Double

    Private dpdIdleDurationPresent As RFmxSpecAnMXDpdReferenceWaveformIdleDurationPresent =
        RFmxSpecAnMXDpdReferenceWaveformIdleDurationPresent.False
    Private dpdSignalType As RFmxSpecAnMXDpdSignalType = RFmxSpecAnMXDpdSignalType.Modulated
    Private numberOfSegments As Integer = 4
	Private linearMemoryDepth As Integer = 21
    Private nonLinearMemoryDepth As Integer = 2
	Private dpdDvrDdrEnabled As RFmxSpecAnMXDpdDvrDdrEnabled = RFmxSpecAnMXDpdDvrDdrEnabled.True
    Private measurementInterval As Double = 0.0001            ' seconds 
    Private dpdDvrNmseEnabled As RFmxSpecAnMXDpdNmseEnabled = RFmxSpecAnMXDpdNmseEnabled.True
    Private dvrTargetWaveform As String = "IDPD Result"      ' Supported Values : IDPD Result, Custom 
    Private customDvrTagretWaveform As ComplexWaveform(Of ComplexSingle)
    Private dpdMeasurementMode As RFmxSpecAnMXDpdMeasurementMode =
        RFmxSpecAnMXDpdMeasurementMode.ExtractOnly
    Private dpdModel As RFmxSpecAnMXDpdModel = RFmxSpecAnMXDpdModel.DecomposedVectorRotation

    Private ampmIdleDurationPresent As RFmxSpecAnMXAmpmReferenceWaveformIdleDurationPresent =
    ...
    Private referencePowerType As RFmxSpecAnMXAmpmReferencePowerType =
        RFmxSpecAnMXAmpmReferencePowerType.Input
    Private thresholdEnabled As RFmxSpecAnMXAmpmThresholdEnabled =
        RFmxSpecAnMXAmpmThresholdEnabled.True
    ...
    Private predistortedWaveform, normalizePredistortedWaveform As ComplexWaveform(Of ComplexSingle)
    Private dvrNmse As Double
    Private dvrModel As ComplexSingle()
    Private referenceWaveformComplexSingle, waveformWithDpdComplexSingle As ComplexWaveform(Of ComplexSingle)

    Private meanLinearGain, onedBCompressionPoint, meanRmsEvm, gainErrorRange, phaseErrorRange,
        meanPhaseError, amToAMResidual, amToPMResidual, powerOffset As Double
    ...
    Private Sub ConfigureRFmx()
        ...
        specAn.Idpd.Configuration.ConfigureReferenceWaveform("", referenceWaveformComplexSingle,
        ...
        autoLevelMeasurementInterval = referenceWaveformComplexSingle.GetRawData().Length / rfsgIqRate


        If sampleRateMode = RFmxSpecAnMXIdpdMeasurementSampleRateMode.ReferenceWaveform Then
            signalBandwidth = 0.8 * rfsgIqRate
            specAn.AutoLevel("", signalBandwidth, autoLevelMeasurementInterval, autoLevelReferenceLevel)
        ElseIf sampleRateMode = RFmxSpecAnMXIdpdMeasurementSampleRateMode.User Then
            signalBandwidth = 0.8 * sampleRate
            specAn.AutoLevel("", signalBandwidth, autoLevelMeasurementInterval, autoLevelReferenceLevel)
        End If

        specAn.ConfigureReferenceLevel("", autoLevelReferenceLevel + gainExpansion)
    End Sub
    ...
    Private Sub RetrieveResults()
        ...
        For i As Integer = 0 To numberOfIterations - 1
            ...
            Dim precisionTiming As PrecisionWaveformTiming = PrecisionWaveformTiming.CreateWithRegularInterval(
                New PrecisionTimeSpan(dx), New PrecisionTimeSpan(x0))
        ...
        specAn.Dpd.Configuration.ConfigureReferenceWaveform("", referenceWaveformComplexSingle,
            dpdIdleDurationPresent, dpdSignalType)
        specAn.Dpd.Configuration.SetDutAverageInputPower("", dutAverageInputPower)
        specAn.Dpd.Configuration.SetMeasurementMode("", dpdMeasurementMode)
        specAn.Dpd.Configuration.SetModel("", dpdModel)
        specAn.Dpd.Configuration.SetDvrNumberOfSegments("", numberOfSegments)
        specAn.Dpd.Configuration.SetDvrLinearMemoryDepth("", linearMemoryDepth)
        specAn.Dpd.Configuration.SetDvrNonlinearMemoryDepth("", nonLinearMemoryDepth)
        specAn.Dpd.Configuration.SetDvrDdrEnabled("", dpdDvrDdrEnabled)
        specAn.Dpd.Configuration.SetNmseEnabled("", dpdDvrNmseEnabled)
        specAn.Dpd.Configuration.SetMeasurementSampleRateMode("",
            RFmxSpecAnMXDpdMeasurementSampleRateMode.ReferenceWaveform)
        specAn.Dpd.Configuration.SetMeasurementInterval("", measurementInterval)

        If Equals(dvrTargetWaveform, "IDPD Result") Then
            specAn.Dpd.Configuration.ConfigureExtractModelTargetWaveform("", predistortedWaveform)
            specAn.Dpd.Configuration.SetMeasurementSampleRate("", 1 / predistortedWaveform.PrecisionTiming.SampleInterval.TotalSeconds)
        ElseIf Equals(dvrTargetWaveform, "Custom") Then
            specAn.Dpd.Configuration.ConfigureExtractModelTargetWaveform("", customDvrTagretWaveform)
            specAn.Dpd.Configuration.SetMeasurementSampleRate("", 1 / customDvrTagretWaveform.PrecisionTiming.SampleInterval.TotalSeconds)
        End If

        specAn.Initiate("", "")

        specAn.Dpd.ApplyDpd.ApplyDigitalPredistortion("", referenceWaveformComplexSingle,
            RFmxSpecAnMXDpdApplyDpdIdleDurationPresent.False, 10.0, waveformWithDpdComplexSingle, papr, powerOffset)

        specAn.Dpd.Results.FetchNmse("", timeout, dvrNmse)
        specAn.Dpd.Results.FetchDvrModel("", timeout, dvrModel)
        ...
        specAn.Ampm.Configuration.ConfigureReferenceWaveform("", referenceWaveformComplexSingle,
            ampmIdleDurationPresent, ampmSignalType)
        ...
        specAn.Ampm.Configuration.SetMeasurementInterval("", measurementInterval)
    ...
    Private Sub DisplayResults()
        Console.WriteLine("-----------------AMPM Measurement-----------------" & Microsoft.VisualBasic.Constants.vbLf)
        ...
        Console.WriteLine(Microsoft.VisualBasic.Constants.vbLf & "-----------------IDPD Measurement-----------------" & Microsoft.VisualBasic.Constants.vbLf)
        ...
        Next
        Console.WriteLine(Microsoft.VisualBasic.Constants.vbLf & "-----------------DPD Measurement------------------" & Microsoft.VisualBasic.Constants.vbLf)
        Console.WriteLine("DVR NMSE (dB)                    {0}", dvrNmse)
        Console.WriteLine(Microsoft.VisualBasic.Constants.vbLf & "DVR Model Coefficients")
        For i As Integer = 0 To dvrModel.Length - 1
            Console.WriteLine(dvrModel(i))
        Next
    End Sub
