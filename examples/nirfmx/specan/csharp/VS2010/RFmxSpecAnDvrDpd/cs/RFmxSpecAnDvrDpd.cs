...
namespace NationalInstruments.Examples.RFmxSpecAnDvrDpd
{
   public class RFmxSpecAnDvrDpd
   {
      ...
      RFmxSpecAnMXIdpdMeasurementSampleRateMode sampleRateMode =
                                          RFmxSpecAnMXIdpdMeasurementSampleRateMode.ReferenceWaveform;
      double sampleRate = 120e6;                      /* S/s */
      RFmxSpecAnMXIdpdReferenceWaveformIdleDurationPresent idleDurationPresent =
      ...
      RFmxSpecAnMXDpdReferenceWaveformIdleDurationPresent dpdIdleDurationPresent =
                                          RFmxSpecAnMXDpdReferenceWaveformIdleDurationPresent.False;
      RFmxSpecAnMXDpdSignalType dpdSignalType = RFmxSpecAnMXDpdSignalType.Modulated;
      int numberOfSegments = 4;
      int linearMemoryDepth = 21;
      int nonLinearMemoryDepth = 2;
      RFmxSpecAnMXDpdDvrDdrEnabled dpdDvrDdrEnabled = RFmxSpecAnMXDpdDvrDdrEnabled.True;
      double measurementInterval = 100e-6;            /* seconds */
      RFmxSpecAnMXDpdNmseEnabled dpdDvrNmseEnabled = RFmxSpecAnMXDpdNmseEnabled.True;
      string dvrTargetWaveform = "IDPD Result";      /* Supported Values : IDPD Result, Custom */
      ComplexWaveform<ComplexSingle> customDvrTagretWaveform;
      RFmxSpecAnMXDpdMeasurementMode dpdMeasurementMode = RFmxSpecAnMXDpdMeasurementMode.ExtractOnly;
      RFmxSpecAnMXDpdModel dpdModel = RFmxSpecAnMXDpdModel.DecomposedVectorRotation;

      RFmxSpecAnMXAmpmReferenceWaveformIdleDurationPresent ampmIdleDurationPresent =
      ...
      private void ConfigureRfsg()
      {
         ...
         NIRfsgPlayback.RetrieveWaveformSampleRate(instrumentHandle, waveformName, out rfsgIqRate);
      ...
      private void ConfigureRFmx()
      {
         ...
         double reciprocal = 1 / rfsgIqRate;
         autoLevelMeasurementInterval = reciprocal * waveformSize;


         if (sampleRateMode == RFmxSpecAnMXIdpdMeasurementSampleRateMode.ReferenceWaveform)
         {
            signalBandwidth = 0.8 * rfsgIqRate;
            specAn.AutoLevel("", signalBandwidth, autoLevelMeasurementInterval, out autoLevelReferenceLevel);
         }
         else if (sampleRateMode == RFmxSpecAnMXIdpdMeasurementSampleRateMode.User)
         {
      ...
      private void RetrieveResults()
      {
         ...
         specAn.Dpd.Configuration.ConfigureReferenceWaveform("", referenceWaveformComplexSingle, dpdIdleDurationPresent
            , dpdSignalType);
         ...
         specAn.Dpd.Configuration.SetMeasurementSampleRateMode("", RFmxSpecAnMXDpdMeasurementSampleRateMode.ReferenceWaveform);
         specAn.Dpd.Configuration.SetMeasurementInterval("", measurementInterval);

         if (dvrTargetWaveform == "IDPD Result")
         {
         ...
         else if (dvrTargetWaveform == "Custom")
         {
         ...
         specAn.Dpd.ApplyDpd.ApplyDigitalPredistortion("", referenceWaveformComplexSingle, RFmxSpecAnMXDpdApplyDpdIdleDurationPresent.False,
            10.0, ref waveformWithDpdComplexSingle, out papr, out powerOffset);

         specAn.Dpd.Results.FetchNmse("", timeout, out dvrNmse);
         specAn.Dpd.Results.FetchDvrModel("", timeout, ref dvrModel);
      ...
      private void DisplayResults()
      {
         ...
         for (int i = 0; i < dvrModel.Length; i++)
         {
