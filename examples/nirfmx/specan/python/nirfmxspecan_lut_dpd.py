...
9. Configure DPD Model = Lookup Table.
10. Configure Lookup Table Threshold and Step Size.
11. Configure Measurement Sample Rate and Interval.
12. Auto Level and configure reference level.
13. Initiate DPD measurement.
14. Configure LUT correction type and apply digital predistortion.
15. Abort RFSG, write predistorted waveform to RFSG, and re-initiate generation.
16. Fetch DPD Lookup Table.
17. Select and configure AMPM measurement. Initiate and fetch results.
18. Close sessions.
...
def example(resource_name, rfsg_resource_name, option_string, reference_waveform_file):
    ...
    lut_step_size = 0.1  # dB
    threshold_level = -20.0  # dB
    threshold_enabled = nirfmxspecan.DpdLookupTableThresholdEnabled.TRUE
    threshold_type = nirfmxspecan.DpdLookupTableThresholdType.RELATIVE
    ...
    try:
        ...
        specan.configure_rf("", center_frequency, 0.0, external_attenuation)
        ...
        specan.dpd.configuration.configure_dpd_model("", nirfmxspecan.DpdModel.LOOKUP_TABLE)
        specan.dpd.configuration.configure_lookup_table_threshold("", threshold_enabled, threshold_level, threshold_type)
        specan.dpd.configuration.configure_lookup_table_step_size("", lut_step_size)
        ...
        auto_ref_level, error_code = specan.auto_level("", auto_level_bandwidth, auto_level_interval)
        specan.configure_reference_level("", auto_ref_level)
        print(f"Reference Level (dBm)    {auto_ref_level:.4f}")

        specan.initiate("", "")

        specan.dpd.apply_dpd.configure_lookup_table_correction_type(
            "", nirfmxspecan.DpdApplyDpdLookupTableCorrectionType.MAGNITUDE_AND_PHASE
        )
        ...
        complex_gains = numpy.zeros(0, dtype=numpy.complex64)
        input_powers, error_code = specan.dpd.results.fetch_lookup_table("", timeout, complex_gains)
        ...
        specan.ampm.configuration.configure_measurement_interval("", auto_level_interval)
...
def _main(argsv):
    parser = argparse.ArgumentParser(
        description="Pass arguments for LUT DPD Example",
