...
7. Configure Measurement Mode to RawIQ and Acquisition parameters.
...
def example(resource_name, option_string):
    ...
    try:
        ...
        specan.iq.configuration.set_measurement_mode("", nirfmxspecan.IQMeasurementMode.RAWIQ)
        specan.iq.configuration.configure_acquisition("", sample_rate, 1, acquisition_time, 0)
        specan.initiate("", "")

        # Fetch Raw IQ data
        data = numpy.array([], dtype=numpy.complex64)
        t0, dt, error_code = instr_session.fetch_raw_iq_data(
...
def _main(argsv):
    parser = argparse.ArgumentParser(
        description="Pass arguments for IQ Raw IQ Example",
