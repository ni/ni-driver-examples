...
//14[A-E]. Results Average Stability, Per Pulse Stability Measurements, Pulse Indices, Burst Selected Position Stability Traces and Pulse to Pulse Stability Traces. 
...
int main(int argc, char* argv[])
{
 ...
 char* digitalEdgeSource = RFMXPULSE_VAL_DIGITAL_EDGE_TRIGGER_SOURCE_PXI_TRIG0_STR;
 float64 triggerDelay = 0.0;                                                   /* (s) */
 int32 digitalEdgeEnable = RFMXPULSE_VAL_TRUE;
 int32 digitalEdge = RFMXPULSE_VAL_DIGITAL_EDGE_RISING_EDGE;
 ...
 int32 pulseMultiburstEnabled = RFMXPULSE_VAL_MULTIBURST_ENABLED_TRUE;
 int32 pulseMultiburstLength = 10;                                            /*Number of Pulse */
 ...
 int32* burstIndex = NULL;
 int32* pulsePositionIndex = NULL;
 ...
 float32* pulseToPulseAmplitudeStability = NULL;                               /* (dB) */
 float32* pulseToPulsePhaseStability = NULL;                                   /* (dB) */
 float32* pulseToPulseTotalStability = NULL;                                   /* (dB) */
 ...
 RFmxCheckWarn(RFmxPulse_CfgDigitalEdgeTrigger(instrumentHandle, "", digitalEdgeSource, digitalEdge, triggerDelay, digitalEdgeEnable));
 ...
 RFmxCheckWarn(RFmxPulse_PulseSetMultiburstEnabled(instrumentHandle, "", pulseMultiburstEnabled));
 RFmxCheckWarn(RFmxPulse_PulseSetMultiburstLength(instrumentHandle, "", pulseMultiburstLength));
 ...
 actualArraySize = 0;
 RFmxCheckWarn(RFmxPulse_PulseGetResultsBurstIndex(instrumentHandle, "", NULL, 0, &actualArraySize));
 if (actualArraySize > 0)
 {
  burstIndex = (float64*)malloc(sizeof(float64) * actualArraySize);
  if (burstIndex)
  {
   RFmxCheckWarn(RFmxPulse_PulseGetResultsBurstIndex(instrumentHandle, "", burstIndex,
	actualArraySize, NULL));
  }
  else
  {
 ...
 RFmxCheckWarn(RFmxPulse_PulseGetResultsPulsePositionIndex(instrumentHandle, "", NULL, 0, &actualArraySize));
 if (actualArraySize > 0)
 {
  pulsePositionIndex = (float64*)malloc(sizeof(float64) * actualArraySize);
  if (pulsePositionIndex)
  {
   RFmxCheckWarn(RFmxPulse_PulseGetResultsPulsePositionIndex(instrumentHandle, "", pulsePositionIndex,
	actualArraySize, NULL));
  }
  else
  {
  ...
  actualArraySize = 0;
 RFmxCheckWarn(RFmxPulse_FetchBurstSelectedPositionStabilityTrace(instrumentHandle, "", timeout, NULL, NULL, NULL, NULL, NULL, 0,
  &actualArraySize));
 if (actualArraySize > 0)
 {
  ...
  {
   RFmxCheckWarn(RFmxPulse_FetchBurstSelectedPositionStabilityTrace(instrumentHandle, "", timeout, &x0, &dx,
	pulseAmplitudeStability, pulsePhaseStability, pulseTotalStability, actualArraySize, NULL));
  }
  else
  {
   printf("malloc failed.\n");
 ...
 actualArraySize = 0;
 RFmxCheckWarn(RFmxPulse_FetchPulseToPulseStabilityTrace(instrumentHandle, "", timeout, NULL, NULL, NULL, NULL, NULL, 0,
  &actualArraySize));
 if (actualArraySize > 0)
 {
   pulseToPulseAmplitudeStability = (float32*)malloc(sizeof(float32) * actualArraySize);
   pulseToPulsePhaseStability = (float32*)malloc(sizeof(float32) * actualArraySize);
   pulseToPulseTotalStability = (float32*)malloc(sizeof(float32) * actualArraySize);

  if (pulseAmplitudeStability && pulsePhaseStability && pulseTotalStability)
  {
   RFmxCheckWarn(RFmxPulse_FetchPulseToPulseStabilityTrace(instrumentHandle, "", timeout, &x0, &dx, pulseToPulseAmplitudeStability,
    pulseToPulsePhaseStability, pulseToPulseTotalStability, actualArraySize, NULL));
  }
  else
  {
   printf("malloc failed.\n");
 ...
 {
  printf("Index                                  : %d\n", i);
  printf("Burst Index                            : %i\n", burstIndex[i]);
  printf("Pulse Position Index                   : %i\n", pulsePositionIndex[i]);
