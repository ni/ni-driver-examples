...
int main (int argc, char *argv[])
{
	...
	char slotString[MAX_SELECTOR_STRING] = {0};
	...
	slotConfiguration_t slotConfigurationInput[ NUMBER_OF_SLOTS ];

    int32 numberOfTimeslots = NUMBER_OF_SLOTS;
	...
	float64 slotMinimumPower[NUMBER_OF_SLOTS] = {0};
    float64 slotMaximumPower[NUMBER_OF_SLOTS] = {0};
    float64 slotAveragePower[NUMBER_OF_SLOTS] = {0};
    float64 slotBurstWidth[NUMBER_OF_SLOTS] = {0};
    int32 slotMeasurementStatus[NUMBER_OF_SLOTS] = {RFMXGSM_VAL_PVT_MEASUREMENT_STATUS_FAIL};
    float64 slotBurstThreshold[NUMBER_OF_SLOTS] = {0};
	...
	{
		slotConfigurationInput[i].modulationType = RFMXGSM_VAL_MODULATION_TYPE_8PSK ;
		slotConfigurationInput[i].burstType = RFMXGSM_VAL_BURST_TYPE_NB;
		slotConfigurationInput[i].HBFilterWidth = RFMXGSM_VAL_HB_FILTER_WIDTH_NARROW;
		slotConfigurationInput[i].TSC = RFMXGSM_VAL_TSC0;
    ...
    RFmxCheckWarn(RFmxGSM_CfgIQPowerEdgeTrigger(instrumentHandle, "", "0", RFMXGSM_VAL_IQ_POWER_EDGE_RISING_SLOPE, IQPowerEdgeLevel, 
								                triggerDelay,  minimumQuietTimeMode, minimumQuietTime, RFMXGSM_VAL_IQ_POWER_EDGE_TRIGGER_LEVEL_TYPE_RELATIVE,
												enableTrigger));
    ...
    for( i = 0; i < NUMBER_OF_SLOTS; i++ )
	{
		RFmxCheckWarn(RFmxGSM_BuildSlotString("", i, MAX_SELECTOR_STRING, slotString));
		RFmxCheckWarn(RFmxGSM_CfgSignalType(instrumentHandle, slotString, slotConfigurationInput[i].modulationType, 
											slotConfigurationInput[i].burstType, slotConfigurationInput[i].HBFilterWidth));
        RFmxCheckWarn(RFmxGSM_CfgTSC(instrumentHandle, slotString, slotConfigurationInput[i].TSC));
        RFmxCheckWarn(RFmxGSM_CfgPowerControlLevel(instrumentHandle, slotString, slotConfigurationInput[i].powerControlLevel));
    }
	...
	RFmxCheckWarn(RFmxGSM_PVTFetchSlotMeasurementArray(instrumentHandle, "", timeout, slotAveragePower, slotBurstWidth, 
								slotMeasurementStatus, slotMaximumPower, slotMinimumPower, slotBurstThreshold, NUMBER_OF_SLOTS, NULL));
	...
	for( i = 0; i < NUMBER_OF_SLOTS; i++ )
	{
		...
		printf("Measurement Status       : %s\n",slotMeasurementStatus[i] == RFMXGSM_VAL_PVT_SLOT_MEASUREMENT_STATUS_PASS?"Pass":"Fail");
	}
