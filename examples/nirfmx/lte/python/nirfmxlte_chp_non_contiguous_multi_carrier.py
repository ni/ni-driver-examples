"""
RFmx LTE CHP Non-Contiguous Multi-Carrier Example
...
5. Configure Subblock Parameters:
...
def example(resource_name, option_string):
    """LTE CHP non-contiguous multi-carrier measurement example."""
    ...
    try:
        ...
        for i in range(number_of_subblocks):
            ...
            subblock_power, integration_bandwidth, frequency, error_code = (
                lte_signal.chp.results.fetch_subblock_measurement(subblock_string, timeout)
            )
            (
                absolute_power,
                relative_power,
                error_code,
            ) = lte_signal.chp.results.component_carrier.fetch_measurement_array(
        ...
        total_aggregated_power, error_code = lte_signal.chp.results.fetch_total_aggregated_power(
            "", timeout
        )
        ...
        for i, res in enumerate(subblock_results):
            ...
            for j in range(len(res["absolute_power"])):
                print(f"Carrier : {j}")
                print(f"Absolute Power (dBm)                        : {res['absolute_power'][j]}")
                print(f"Relative Power (dB)                         : {res['relative_power'][j]}")
...
def _main(argsv):
    ...
    parser = argparse.ArgumentParser(
        description="Pass arguments for LTE CHP Non-Contiguous Multi-Carrier Example",
