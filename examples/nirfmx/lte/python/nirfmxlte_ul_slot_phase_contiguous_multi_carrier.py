"""
RFmx LTE UL Slot Phase Contiguous Multi-Carrier Example
...
9. Configure Synchronization Mode and Interval.
...
def example(resource_name, option_string):
    """LTE UL slot phase contiguous multi-carrier measurement example."""
    ...
    try:
        ...
        maximum_phase_discontinuity, error_code = (
            lte_signal.slotphase.results.fetch_maximum_phase_discontinuity_array("", timeout)
        )

        for i in range(number_of_component_carriers):
            carrier_string = nirfmxlte.Lte.build_carrier_string("", i)
            slot_phase_discontinuity = numpy.empty(0, dtype=numpy.float64)
        ...
        for i in range(number_of_component_carriers):
            carrier_string = nirfmxlte.Lte.build_carrier_string("", i)
            print(f"\nCarrier {i}")
            print(f"Maximum Phase Discontinuity (deg)   : {maximum_phase_discontinuity[i]}")
...
def _main(argsv):
    ...
    parser = argparse.ArgumentParser(
        description="Pass arguments for LTE UL Slot Phase Contiguous Multi-Carrier Example",
