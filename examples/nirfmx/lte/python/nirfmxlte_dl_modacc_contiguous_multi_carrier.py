"""
RFmx LTE DL ModAcc Contiguous Multi-Carrier Example
...
def example(resource_name, option_string):
    """LTE DL ModAcc contiguous multi-carrier measurement example."""
    ...
    downlink_test_model = [nirfmxlte.DownlinkTestModel.TM_1_1, nirfmxlte.DownlinkTestModel.TM_1_1]
    ...
    try:
        ...
        lte_signal.component_carrier.configure_downlink_test_model_array("", downlink_test_model)
        ...
        (
            ...
            mean_rms_256qam_evm,
            error_code,
        ) = lte_signal.modacc.results.fetch_pdsch_evm_array("", timeout)

        mean_rms_1024qam_evm, error_code = lte_signal.modacc.results.fetch_pdsch_1024_qam_evm_array("", timeout)
        ...
        for i in range(number_of_component_carriers):
            ...
            print(f"Mean RMS EVM  (% or dB)              : {mean_rms_evm[i]}")
            print(f"Mean RMS QPSK EVM  (% or dB)         : {mean_rms_qpsk_evm[i]}")
            print(f"Mean RMS 16QAM EVM  (% or dB)        : {mean_rms_16qam_evm[i]}")
            print(f"Mean RMS 64QAM EVM  (% or dB)        : {mean_rms_64qam_evm[i]}")
            print(f"Mean RMS 256QAM EVM  (% or dB)       : {mean_rms_256qam_evm[i]}")
            print(f"Mean RMS 1024QAM EVM  (% or dB)      : {mean_rms_1024qam_evm[i]}")
...
def _main(argsv):
    ...
    parser = argparse.ArgumentParser(
        description="Pass arguments for LTE DL ModAcc Contiguous Multi-Carrier Example",
