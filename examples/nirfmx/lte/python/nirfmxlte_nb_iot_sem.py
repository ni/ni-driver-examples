...
def example(resource_name, option_string):
    """LTE NB-IoT SEM measurement example."""
    ...
    # Downlink-specific parameters (used when link_direction = DOWNLINK)
    ...
    try:
        ...
        x0, dx, error_code = lte_signal.sem.results.fetch_spectrum("", timeout, spectrum, composite_mask)

        # Print results
        print(f"Measurement Status                         :{measurement_status.name}")
        print(f"Carrier Absolute Integrated Power (dBm)    :{absolute_integrated_power}")
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="RFmx LTE NB-IoT SEM Example",
