...
int main(int argc, char *argv[])
{
   ...
   int32 laaNumberOfSubframes[NUMBER_OF_COMPONENT_CARRIERS] = { 1, 1 };
   int32 laaStartingSubframe[NUMBER_OF_COMPONENT_CARRIERS] = { 0, 0 };
   int32 laaUplinkStartPosition[NUMBER_OF_COMPONENT_CARRIERS] = { RFMXLTE_VAL_LAA_UPLINK_START_POSITION_00,
                                                                 RFMXLTE_VAL_LAA_UPLINK_START_POSITION_00 };
   int32 laaUplinkEndingSymbol[NUMBER_OF_COMPONENT_CARRIERS] = { RFMXLTE_VAL_LAA_UPLINK_ENDING_SYMBOL_13,
                                                                RFMXLTE_VAL_LAA_UPLINK_ENDING_SYMBOL_13 };
   ...
   RFmxCheckWarn(RFmxLTE_CfgBand(instrumentHandle, "", 46));
   RFmxCheckWarn(RFmxLTE_CfgDuplexScheme(instrumentHandle, "", RFMXLTE_VAL_DUPLEX_SCHEME_LAA, uplinkDownlinkConfiguraiton));
   ...
   for (i = 0; i < NUMBER_OF_COMPONENT_CARRIERS; i++)
   {
      ...
      RFmxCheckWarn(RFmxLTE_CfgComponentCarrier(instrumentHandle, subblockCarrierString[i],
         componentCarrierBandwidth[i], componentCarrierFrequency[i], componentCarrierCellId[i]));
      RFmxCheckWarn(RFmxLTE_SetLAAStartingSubframe(instrumentHandle, subblockCarrierString[i],
         laaStartingSubframe[i]));
      RFmxCheckWarn(RFmxLTE_SetLAANumberOfSubframes(instrumentHandle, subblockCarrierString[i],
         laaNumberOfSubframes[i]));
      RFmxCheckWarn(RFmxLTE_SetLAAUplinkStartPosition(instrumentHandle, subblockCarrierString[i],
         laaUplinkStartPosition[i]));
      RFmxCheckWarn(RFmxLTE_SetLAAUplinkEndingSymbol(instrumentHandle, subblockCarrierString[i],
         laaUplinkEndingSymbol[i]));
   }
   ...
   RFmxCheckWarn(RFmxLTE_ModAccFetchCompositeEVMArray(instrumentHandle, "", timeout, meanRMSCompositeEVM,
      maximumPeakCompositeEVM, meanFrequencyError, peakCompositeEVMSymbolIndex, peakCompositeEVMSubcarrierIndex,
      peakCompositeEVMSlotIndex, NUMBER_OF_COMPONENT_CARRIERS, NULL));
   ...
   for (i = 0; i < NUMBER_OF_COMPONENT_CARRIERS; i++)
   {
      ...
      printf("Maximum Peak Composite EVM  (%% or dB)    : %lf\n", maximumPeakCompositeEVM[i]);
