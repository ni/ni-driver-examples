...
int main(int argc, char *argv[])
{
   ...
   float64 componentCarrierMaximumOutputPower[NUMBER_OF_COMPONENT_CARRIERS] = { 0.0, 0.0 }; /* (dBm) */
   ...
   RFmxCheckWarn(RFmxLTE_CfgComponentCarrierSpacing(instrumentHandle, "",
      componentCarrierSpacingType, componentCarrierAtCenterFrequency));
   ...
   else if (linkDirection == RFMXLTE_VAL_LINK_DIRECTION_DOWNLINK)
   {
      ...
      RFmxCheckWarn(RFmxLTE_SEMCfgComponentCarrierMaximumOutputPowerArray(instrumentHandle, "",
         componentCarrierMaximumOutputPower, NUMBER_OF_COMPONENT_CARRIERS));
   }
   else
   ...
   if (actualArraySize > 0)
   {
      ...
      if (lowerOffsetMeasurementStatus && lowerOffsetMargin && lowerOffsetMarginFrequency &&
         ...
         RFmxCheckWarn(RFmxLTE_SEMFetchLowerOffsetMarginArray(instrumentHandle, "", timeout, lowerOffsetMeasurementStatus,
            ...
            lowerOffsetMarginRelativePower, actualArraySize, &arraySize));
      else
      {
         printf("malloc failed.\n");
         goto Error;
