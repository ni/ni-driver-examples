"""
RFmx VNA S-Parameters Corrected (One-Port) Example
...
def example(resource_name, option_string):
    """VNA S-parameters corrected one-port example."""
    ...
    # Two traces on S11: magnitude and phase
    number_of_sparams = 2
    sparams_parameters = ["S11", "S11"]
    sparams_formats = [
        nirfmxvna.SParamsFormat.MAGNITUDE,
        nirfmxvna.SParamsFormat.PHASE,
    ]
    ...
    try:
        ...
        vna_signal.set_correction_calibration_method("", nirfmxvna.CorrectionCalibrationMethod.SOL)

        # Use "port::all" to apply to all calibration ports
        ...
        print("Connect DUT to the calibrated port of NI PXIe-5633 and terminate the unused port.")
        input("Press any key to continue.")
...
def _parse_args():
    parser = argparse.ArgumentParser(
        description="VNA S-Parameters Corrected (One-Port) Example",
