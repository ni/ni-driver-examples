"""
RFmx VNA Pulse Generators Example
...
def example(resource_name, option_string):
    """VNA pulse generators example."""
    frequency_start = 1e9  # Hz
    frequency_stop = 1e9  # Hz (CW — single frequency)
    number_of_frequency_points = 1
    ...
    number_of_sparams = 1
    sparams_parameters = ["S11"]
    sparams_formats = [nirfmxvna.SParamsFormat.MAGNITUDE]

    pulse_mode_enabled = nirfmxvna.PulseModeEnabled.TRUE
    pulse_period = 1.0e-3  # s
    pulse_modulator_delay = 0.0  # s
    pulse_modulator_width = 100.0e-6  # s
    pulse_acquisition_auto = nirfmxvna.PulseAcquisitionAuto.TRUE
    pulse_acquisition_delay = 46.65e-6  # s
    pulse_acquisition_width = 20.0e-6  # s

    number_of_pulse_generators = 4
    pulse_generator_enabled = [
        nirfmxvna.PulseGeneratorEnabled.TRUE,
        nirfmxvna.PulseGeneratorEnabled.FALSE,
        nirfmxvna.PulseGeneratorEnabled.FALSE,
        nirfmxvna.PulseGeneratorEnabled.FALSE,
    ]
    pulse_generator_export_output_terminal = [
        "PFI0",               # RFmxInstrMXConstants.Pfi0
        "DoNotExportSignal",  # RFmxInstrMXConstants.DoNotExportSignal
        "DoNotExportSignal",
        "DoNotExportSignal",
    ]
    pulse_generator_delay = [0.0, 0.0, 0.0, 0.0]  # s
    pulse_generator_width = [100.0e-6, 100.0e-6, 100.0e-6, 100.0e-6]  # s
    ...
    try:
        ...
        for i in range(number_of_pulse_generators):
            pulse_gen_selector_string = nirfmxvna.Vna.build_pulse_generator_string("", i)
            vna_signal.set_pulse_generator_enabled(pulse_gen_selector_string, pulse_generator_enabled[i])
            vna_signal.set_pulse_generator_export_output_terminal(
                pulse_gen_selector_string, pulse_generator_export_output_terminal[i]
            )
            vna_signal.set_pulse_generator_delay(pulse_gen_selector_string, pulse_generator_delay[i])
            vna_signal.set_pulse_generator_width(pulse_gen_selector_string, pulse_generator_width[i])
...
def _parse_args():
    parser = argparse.ArgumentParser(
        description="VNA Pulse Generators Example",
