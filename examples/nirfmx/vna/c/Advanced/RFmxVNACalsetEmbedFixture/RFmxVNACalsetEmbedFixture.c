...
// 3 & 4. Configure S-parameter External Attenuation Table (External Fixture's De-embedding Table) from S2P File and set Power Compensation.
// 5. Configure sweep settings: Sweep Type, Start Frequency, Stop Frequency, Number of Frequency Points,  IF Bandwidth, and Power Level and Test Rx Attenuation with different port names.
...
int main(int argc, char* argv[])
{
    ...
    int32 sParameterOrientations[NUMBER_OF_EXTERNAL_FIXTURES] =
        {
            ...
            RFMXINSTR_VAL_PORT2_TOWARDS_DUT};
    int32 powerCompensationEnabled[NUMBER_OF_EXTERNAL_FIXTURES] =
        {
            RFMXINSTR_VAL_FALSE,
            RFMXINSTR_VAL_FALSE};
    ...
    for (index = 0; index < NUMBER_OF_EXTERNAL_FIXTURES; index++) {
        ...
        RFmxCheckWarn(RFmxInstr_CfgSParameterExternalAttenuationPowerCompensationEnabled(instrumentHandle, portSelectorString, powerCompensationEnabled[index]));
    }
