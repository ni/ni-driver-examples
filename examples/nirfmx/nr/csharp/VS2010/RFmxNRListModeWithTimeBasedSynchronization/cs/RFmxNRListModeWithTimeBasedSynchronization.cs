...
namespace NationalInstruments.Examples.RFmxNRListModeWithTimeBasedSynchronization
{
   public class RFmxNRListModeWithTimeBasedSynchronization
   {
      ...
      RFmxNRMXListStepTimerUnit listStepTimerUnit;
      ...
      void ConfigureRFmx()
      {
         ...
         for (int i = 0; i < numberOfSteps; i++)
         {
            ...
            if(i == 0)
            {
                step[i].ConfigureIQPowerEdgeTrigger("", "0", RFmxNRMXIQPowerEdgeTriggerSlope.Rising, step0IQPowerEdgeLevel,
                    triggerDelay, RFmxNRMXTriggerMinimumQuietTimeMode.Manual, step0MinimumQuietTime,
                    RFmxNRMXIQPowerEdgeTriggerLevelType.Relative, true);
            }
            else
            {
                step[i].ConfigureDigitalEdgeTrigger("", "TimerEvent", RFmxNRMXDigitalEdgeTriggerEdge.Rising,
                    triggerDelay, true);
