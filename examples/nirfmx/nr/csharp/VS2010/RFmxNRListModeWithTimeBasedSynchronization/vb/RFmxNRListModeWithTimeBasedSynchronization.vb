...
'24. Configure Link Direction, Frequency Range, Carrier Bandwidthand Subcarrier Spacing for all List Step.
'25. Configure Sweep Time Parameters for all List Step.
'26. Select ACP measurement and enable Traces for all List Step.
'27. Initiate ACP measurement for NR List.
...
Namespace NationalInstruments.Examples.RFmxNRListModeWithTimeBasedSynchronization
	Public Class RFmxNRListModeWithTimeBasedSynchronization
		...
		Private listStepTimerUnit As RFmxNRMXListStepTimerUnit
		...
		Private Sub InitializeVariables()
			...
			rfsgSelectedPorts = ""
			waveformFilePath = "NR_FR1_UL_BW-100MHz_SCS-30kHz.tdms"
			waveformName = "Wfm"
			...
			listStepTimerUnit = RFmxNRMXListStepTimerUnit.Time
			listStepTimerDuration = 0.0015
			' (s) 
		...
		Private Sub ConfigureRFmx()
			...
			For i As Integer = 0 To numberOfSteps - 1
				...
				If i = 0 Then
					[step](i).ConfigureIQPowerEdgeTrigger("", "0", RFmxNRMXIQPowerEdgeTriggerSlope.Rising, step0IQPowerEdgeLevel, triggerDelay, RFmxNRMXTriggerMinimumQuietTimeMode.Manual,
					step0MinimumQuietTime, RFmxNRMXIQPowerEdgeTriggerLevelType.Relative, True)
				Else
					[step](i).ConfigureDigitalEdgeTrigger("", "TimerEvent", RFmxNRMXDigitalEdgeTriggerEdge.Rising, triggerDelay, True)
			...
			stepAll.SelectMeasurements("", RFmxNRMXMeasurementTypes.Acp, True)
			stepAll.Acp.Configuration.ConfigureSweepTime("", RFmxNRMXAcpSweepTimeAuto.[False], sweepTimeInterval)
