//===================================================================
//
// Title:       Read Waveform from File.c
...
char script[1024], fileName[1024];
...
int32 chnNumber, standard, frequencyBand, outputPort;
int32 waveformSize, actWaveformSize, eof;

NIComplexNumber *dataWaveform = NULL;           
	
float64 upConverterCenterFrequency, t0, dt, extIQCarFreq;
float64 upConverterCenterFrequencyOffset, externalAttenuation;
float64 carrierFrequency, powerLevel, actualHeadroom, IQRate;

int32 terminalConfiguration, outPort;
...
int32 readWaveformAndGenerate(void);
...
int main (int argc, char *argv[])
{
	...
	checkWarn(readWaveformAndGenerate());
...
int32 configureRfsgSession(void)
{
	...
	if ((!strcmp(model,"NI PXIe-5644R"))||(!strcmp(model,"NI PXIe-5645R"))||(!strcmp(model,"NI PXIe-5646R"))) 
	{
...
/*--------------------------------------------------------------------------*/
/* Function to read waveform and generate		                            */
/*--------------------------------------------------------------------------*/
int32 readWaveformAndGenerate(void)
{
	int32 error = 0;
	...
	/*-----------------------------------------------------------------------------------------------*/
	/* TDMS File Operations																			 */
	/*-----------------------------------------------------------------------------------------------*/
	checkWarn(niBTSG_ReadWaveformFromFile(fileName, NULL, 0, -1, NULL, NULL, NULL, 0, &waveformSize, NULL, NULL, NULL));		
	checkAlloc(dataWaveform = (NIComplexNumber *) malloc(waveformSize * sizeof(NIComplexNumber)));
	checkWarn(niBTSG_ReadWaveformFromFile(fileName, NULL, 0, -1, &t0, &dt, dataWaveform, waveformSize, &actWaveformSize, 
										  &IQRate, &actualHeadroom, &eof));		

	checkWarn(niRFSG_WriteArbWaveformComplexF64(rfsgSession, waveformName, actWaveformSize, dataWaveform, NIBTSG_VAL_TRUE));

    checkWarn(niBTSG_RFSGStoreIQRate(rfsgSession, NULL, waveformName, IQRate));
	checkWarn(niBTSG_RFSGStoreHeadroom(rfsgSession, NULL, waveformName, actualHeadroom));
