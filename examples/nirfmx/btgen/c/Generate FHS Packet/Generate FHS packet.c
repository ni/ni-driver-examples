//===================================================================
//
// Title:       Generate FHS packet.c
// Purpose:     Example to Generate FHS packet
//
// 
...
int32 fhsBDaddrLAP, fhsBDaddrUAP, fhsBDaddrNAP;
...
int32 initGlobalVaribales(void)
{
	...
	strcpy(waveformName, "FHS");

	sprintf(script, "%s", "script GenerateFHSPkt\nrepeat forever\ngenerate FHS\ngenerate idle\nend repeat\nend script");
...
int32 configureToolkitSession(void)
{
	...
	/* Set Packet Type */
	checkWarn(niBTSG_SetPacketType(gBTSession, "", NIBTSG_VAL_PACKET_TYPE_FHS));
	...
	/* FHS Payload BD Address */
	checkWarn(niBTSG_SetFHSPayloadBDAddress(gBTSession, "", fhsBDaddrLAP, fhsBDaddrUAP, fhsBDaddrNAP));
	checkWarn(niBTSG_SetFHSPayloadLTAddress(gBTSession, "", fhsLTAddress));
	checkWarn(niBTSG_SetFHSPayloadDeviceClass(gBTSession, "", fhsDevClass));
	checkWarn(niBTSG_SetFHSPayloadScanRepetition(gBTSession, "", fhsScanRep));
	checkWarn(niBTSG_SetFHSPayloadPageScanMode(gBTSession, "", fhsPageScan));
	checkWarn(niBTSG_SetFHSPayloadDeviceClock(gBTSession, "", fhsDevClock));
