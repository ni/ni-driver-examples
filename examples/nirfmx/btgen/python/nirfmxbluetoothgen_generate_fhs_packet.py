r"""Bluetooth Generate FHS Packet Example.
...
4. Configure Packet Type (FHS) and Carrier Mode.
5. Configure Packet Header (LT Address, Flow, ARQN, SEQN).
6. Configure FHS Payload (BD Address, LT Address, Device Class, Scan Repetition,
   Page Scan Mode, Device Clock).
7. Configure Waveform Properties (Headroom, Carrier Frequency Offset).
8. Configure IQ Impairments.
9. Configure AWGN.
10. Configure Whitening.
11. Configure RFSG session (Frequency, Power Level, External Attenuation).
12. Create and Download FHS Packet Waveform and IDLE Waveform to RFSG.
...
def example(resource_name, option_string):
    """Run Bluetooth Generate FHS Packet Example."""
    ...
    # FHS Payload
    fhs_payload_bd_address_lap = 0
    fhs_payload_bd_address_uap = 0
    fhs_payload_bd_address_nap = 0
    fhs_payload_lt_address = 0
    fhs_payload_device_class = 0
    fhs_payload_scan_repetition = 0
    fhs_payload_page_scan_mode = 0
    fhs_payload_device_clock = 0
    ...
    try:
        ...
        btsg_session.set_packet_type("", nirfmxbluetoothgen.PacketType.FHS.value)

        # Configure Carrier Mode
        ...
        # Configure FHS Payload
        btsg_session.set_fhs_payload_bd_address_lap("", fhs_payload_bd_address_lap)
        btsg_session.set_fhs_payload_bd_address_uap("", fhs_payload_bd_address_uap)
        btsg_session.set_fhs_payload_bd_address_nap("", fhs_payload_bd_address_nap)
        btsg_session.set_fhs_payload_lt_address("", fhs_payload_lt_address)
        btsg_session.set_fhs_payload_device_class("", fhs_payload_device_class)
        btsg_session.set_fhs_payload_scan_repetition("", fhs_payload_scan_repetition)
        btsg_session.set_fhs_payload_page_scan_mode("", fhs_payload_page_scan_mode)
        btsg_session.set_fhs_payload_device_clock("", fhs_payload_device_clock)
        ...
        # Create and Download FHS Packet Waveform
...
def _main(argsv):
    ...
    parser = argparse.ArgumentParser(
        description="Pass arguments for Bluetooth Generate FHS Packet Example",
