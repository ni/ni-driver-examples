r"""Bluetooth Generate DV Packet Example.
...
4. Configure Packet Type (DV).
5. Configure Packet Header (LT Address, Flow, ARQN, SEQN).
6. Configure Payload Header (LLID, Flow, Length Mode).
7. Configure Payload Data (Data Type, PN Order, PN Seed).
8. Configure DV Voice Payload (Data Type, PN Order, PN Seed).
9. Configure Waveform Properties (Headroom, Carrier Frequency Offset).
10. Configure IQ Impairments.
11. Configure AWGN.
12. Configure Whitening.
13. Configure RFSG session (Frequency, Power Level, External Attenuation).
14. Create and Download DV Packet Waveform and IDLE Waveform to RFSG.
...
def example(resource_name, option_string):
    """Run Bluetooth Generate DV Packet Example."""
    ...
    # DV Voice Payload
    dv_voice_payload_data_type = nirfmxbluetoothgen.DVVoicePayloadDataType.PN_SEQUENCE
    dv_voice_payload_pn_order = 9
    dv_voice_payload_pn_seed = 497
    ...
    try:
        ...
        btsg_session.set_packet_type("", nirfmxbluetoothgen.PacketType.DV.value)
        ...
        if payload_length_mode != nirfmxbluetoothgen.PayloadLengthMode.MAXIMUM_LENGTH:
        ...
        # Configure DV Voice Payload
        btsg_session.set_dv_voice_payload_data_type("", dv_voice_payload_data_type.value)
        btsg_session.set_dv_voice_payload_pn_order("", dv_voice_payload_pn_order)
        btsg_session.set_dv_voice_payload_pn_seed("", dv_voice_payload_pn_seed)
        ...
        # Create and Download DV Packet Waveform
...
def _main(argsv):
    ...
    parser = argparse.ArgumentParser(
        description="Pass arguments for Bluetooth Generate DV Packet Example",
