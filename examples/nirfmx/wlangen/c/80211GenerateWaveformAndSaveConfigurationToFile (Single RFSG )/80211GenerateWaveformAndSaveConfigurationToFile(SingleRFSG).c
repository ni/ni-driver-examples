 ...
 *---------------------------------------------------------------------------
 *
 * Title:    80211GenerateWaveformAndSaveConfigurationToFile(SingleRFSG).c
 ...
 *			 single-channel 802.11be waveform using an NI vector signal generator. This example also demonstrates
 *           how to save the toolkit configuration to a TDMS file.
 *
Comments:
...
16. Save NI WLAN Generation session configuration to a .tdms file.
...
Error:
   ...
   /*Close WLAN session*/
	if(gSession)
	 checkWarn(niWLANG_CloseSession(gSession));
...
int32 createAndDownloadWaveform(void)
{
	...
	checkWarn(niWLANG_SaveConfigurationToFile(gSession,filePath, NIWLANG_VAL_FILE_OPERATION_MODE_CREATE_OR_REPLACE));
