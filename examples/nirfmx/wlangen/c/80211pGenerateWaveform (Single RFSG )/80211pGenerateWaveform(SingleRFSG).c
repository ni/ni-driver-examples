 ...
 *---------------------------------------------------------------------------
 *
 * Title:    80211pGenerateWaveform(SingleRFSG).c
 * Purpose:  This example demonstrates Generation of an 802.11p waveform using an NI vector signal generator.
 *
 Comments:
...
3. Configure  frequency of generator.
4. Open an NI WLAN Generation session.
5. Configure basic WLAN generation properties.
    - Set Standard to 80211P OFDM.
    - Set pulse shaping filter properties.
...
10. 
    ...
    B. Exit if an error has occurred, the Stop button is pressed or generation is done.
11. Abort signal generation.
12. Disable the output. This sets the noise floor as low as possible.
13. Clear the waveform from the device memory. Clear the waveform properties from RFSG database.
14. Close the NI-RFSG session.
15. Read signal properties (waveform size and actual headroom) for display purpose.
16. Close the NI WLAN Generation session.
 ****************************************************************************/
#include <stdio.h>
#include <conio.h>
#include <stdlib.h>
...
static int32 standard,ofdmDataRate;
static int32 pulseShapingFilterLength,pulseShapingFilterEnabled,pulseShapingFilterType;
static float64 pulseShapingFilterParameter;
...
int32 initGlobalVaribales(void)
{
	...
	standard = NIWLANG_VAL_STANDARD_80211P_OFDM;
	channelBandwidth = 20e+6;
	ofdmDataRate = NIWLANG_VAL_OFDM_DATA_RATE_6;
	pulseShapingFilterEnabled = NIWLANG_VAL_TRUE;
	pulseShapingFilterType = NIWLANG_VAL_FILTER_RAISED_COSINE;
	pulseShapingFilterParameter = 0.1;
	pulseShapingFilterLength = 100;
...
int32 configureToolkitSessionForGeneration(void)
{
	...
	checkWarn(niWLANG_SetPulseShapingFilterType(gSession, NULL, pulseShapingFilterType));
	checkWarn(niWLANG_SetPulseShapingFilterParameter(gSession, NULL, pulseShapingFilterParameter));
	checkWarn(niWLANG_SetPulseShapingFilterLength(gSession, NULL, pulseShapingFilterLength));
...
int32 createAndDownloadWaveform(void)
{
	...
	checkWarn(niWLANG_RFSGConfigureFrequencySingleLO(gSession,rfsgSession,1,NIWLANG_VAL_LO_SOURCE_ONBOARD,externalLOSession,carrierFrequency,
	...
	checkWarn(niWLANG_GetActualHeadroom(gSession,NULL, &headroom));
