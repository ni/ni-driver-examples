"""
RFmx WLAN SEM Custom Mask Example
...
def example(resource_name, option_string):
    """WLAN SEM Custom Mask measurement example."""
    ...
    mask_type = nirfmxwlan.SemMaskType.CUSTOM
    ...
    offset_start_frequency = numpy.array([9e6, 11e6, 20e6], dtype=numpy.float64)   # Hz
    offset_stop_frequency = numpy.array([11e6, 20e6, 40e6], dtype=numpy.float64)   # Hz
    offset_sideband = numpy.array(
        [
            nirfmxwlan.SemOffsetSideband.BOTH.value,
            nirfmxwlan.SemOffsetSideband.BOTH.value,
            nirfmxwlan.SemOffsetSideband.BOTH.value,
        ],
        dtype=numpy.int32,
    )
    relative_limit_start = numpy.array([0.0, -20.0, -28.0], dtype=numpy.float64)   # dB
    relative_limit_stop = numpy.array([-20.0, -28.0, -40.0], dtype=numpy.float64)  # dB
    ...
    try:
        ...
        wlan_signal.sem.configuration.configure_number_of_offsets("", number_of_offsets)
        wlan_signal.sem.configuration.configure_offset_frequency_array(
            "", offset_start_frequency, offset_stop_frequency, offset_sideband
        )
        wlan_signal.sem.configuration.configure_offset_relative_limit_array(
...
def _main(argsv):
    ...
    parser = argparse.ArgumentParser(
        description="Pass arguments for WLAN SEM Custom Mask Example",
