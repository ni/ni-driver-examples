...
Namespace NationalInstruments.Examples.RFmxWlanSemCustomMask
    ...
    Public Class RFmxWlanSemCustomMask
        ...
        Const numberOfOffsets As Integer = 3

        Private offsetStartFrequency As Double()
        Private offsetStopFrequency As Double()
        Private offsetSideband As RFmxWlanMXSemOffsetSideband()
        Private relativeLimitStart As Double()
        Private relativeLimitStop As Double()
        ...
        Private Sub InitializeVariables()
            ...
            offsetStartFrequency = New Double(numberOfOffsets - 1) {9000000.0, 11000000.0, 20000000.0}
            offsetStopFrequency = New Double(numberOfOffsets - 1) {11000000.0, 20000000.0, 40000000.0}
            offsetSideband = New RFmxWlanMXSemOffsetSideband(numberOfOffsets - 1) {RFmxWlanMXSemOffsetSideband.Both, RFmxWlanMXSemOffsetSideband.Both, RFmxWlanMXSemOffsetSideband.Both}
            relativeLimitStart = New Double(numberOfOffsets - 1) {0.0, -20.0, -28.0}
            '(dB)

            relativeLimitStop = New Double(numberOfOffsets - 1) {-20.0, -28.0, -40.0}
            '(dB)
        ...
        Private Sub ConfigureWlan()
         ...
         minimumQuietTime, RFmxWlanMXIQPowerEdgeTriggerLevelType.Relative, iqPowerEdgeEnabled)
            ...
            wlan.Sem.Configuration.ConfigureOffsetFrequencyArray("", offsetStartFrequency, offsetStopFrequency, offsetSideband)
        ...
        Private Sub PrintResults()
            ...
            Console.WriteLine(vbLf & "----------Lower Segment Measurements----------" & vbLf)
            ...
            Console.WriteLine(vbLf & "----------Upper Segment Measurements----------" & vbLf)
