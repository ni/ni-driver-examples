...
int main(int argc, char *argv[])
{
    ...
    int32 payloadBitPattern = RFMXBT_VAL_PAYLOAD_BIT_PATTERN_10101010;
    ...
    float64 df2avgMinimum = 0;                                                   /*(Hz) */
    float64 percentageOfSymbolsAbouveDf2maxThreshold = 0;                        /*(%) */

    float64 initialFrequencyErrorMaximum = 0.0;                                  /*(Hz) */
    float64 BRPeakFrequencyDriftMaximum = 0.0;                                   /*(Hz) */
    float64 BRPeakFrequencyDriftRateMaximum = 0.0;                               /*(Hz) */

    float64 peakFrequencyErrorMaximum = 0.0;                                     /*(Hz) */
    float64 initialFrequencyDriftMaximum = 0.0;                                  /*(Hz) */
    float64 LEPeakFrequencyDriftMaximum = 0.0;                                   /*(Hz) */
    float64 LEPeakFrequencyDriftRateMaximum = 0.0;                               /*(Hz) */
    ...
    /* Fetch Results */
    RFmxCheckWarn(RFmxBT_ModAccFetchDf2(instrumentHandle, "", timeout, &df2avgMinimum, &percentageOfSymbolsAbouveDf2maxThreshold));
    RFmxCheckWarn(RFmxBT_ModAccFetchFrequencyErrorBR(instrumentHandle, "", timeout, &initialFrequencyErrorMaximum, &BRPeakFrequencyDriftMaximum, &BRPeakFrequencyDriftRateMaximum));
    RFmxCheckWarn(RFmxBT_ModAccFetchFrequencyErrorLE(instrumentHandle, "", timeout, &peakFrequencyErrorMaximum, &initialFrequencyDriftMaximum, &LEPeakFrequencyDriftMaximum, &LEPeakFrequencyDriftRateMaximum));
    actualArraySize = 0;
    RFmxCheckWarn(RFmxBT_ModAccFetchDf2maxTrace(instrumentHandle, "", timeout, NULL, NULL, 0, &actualArraySize));
    if (actualArraySize > 0)
    {
        time = (float32*)malloc(sizeof(float32) * actualArraySize);
        df2max = (float32*)malloc(sizeof(float32) * actualArraySize);
        if (df2max && time)
        {
            RFmxCheckWarn(RFmxBT_ModAccFetchDf2maxTrace(instrumentHandle, "", timeout, time, df2max,
    ...
    actualArraySize = 0;
    RFmxCheckWarn(RFmxBT_ModAccFetchFrequencyErrorTraceBR(instrumentHandle, "", timeout, NULL, NULL, 0, &actualArraySize));
    if (actualArraySize > 0)
    {
        timeBR = (float32*)malloc(sizeof(float32) * actualArraySize);
        frequencyErrorBR = (float32*)malloc(sizeof(float32) * actualArraySize);
        if (timeBR && frequencyErrorBR)
        {
            RFmxCheckWarn(RFmxBT_ModAccFetchFrequencyErrorTraceBR(instrumentHandle, "", timeout, timeBR, frequencyErrorBR,
                actualArraySize, NULL));
        }
        else
        {
    ...
    printf("------------------df2 Measurement------------------\n");
    printf("df2avg Minimum (Hz)                             : %lf\n", df2avgMinimum);
    printf("Percentage of Symbols above df2max Threshold (%%) : %lf\n", percentageOfSymbolsAbouveDf2maxThreshold);
    printf("\n------------------BR Frequency Error------------------\n\n");
    printf("Initial Frequency Error Maximum (Hz)            : %lf\n", initialFrequencyErrorMaximum);
    printf("Peak Frequency Drift Maximum (Hz)               : %lf\n", BRPeakFrequencyDriftMaximum);
    printf("Peak Frequency Drift Rate Maximum (Hz)          : %lf\n", BRPeakFrequencyDriftRateMaximum);
    printf("\n------------------LE Frequency Error------------------\n\n");
    ...
    printf("Peak Frequency Drift Maximum (Hz)               : %lf\n", LEPeakFrequencyDriftMaximum);
    printf("Peak Frequency Drift Rate Maximum (Hz)          : %lf\n", LEPeakFrequencyDriftRateMaximum);
