...
def example(resource_name, option_string):
    ...
    payload_bit_pattern = nirfmxbluetooth.PayloadBitPattern.PATTERN_11110000
    ...
    try:
        ...
        # Retrieve results
        df1avg_maximum, df1avg_minimum, error_code = bt_signal.modacc.results.fetch_df1(
            "", timeout
        )
        ...
        # Fetch traces
        time, df1max, error_code = bt_signal.modacc.results.fetch_df1max_trace("", timeout)
        ...
        # Print Results
        print("------------------df1 Measurement------------------")
        print(f"df1avg Maximum (Hz)                             : {df1avg_maximum}")
        print(f"df1avg Minimum (Hz)                             : {df1avg_minimum}")
        ...
        print(f"Peak Frequency Drift Maximum (Hz)               : {peak_frequency_drift_maximum}")
        print(
            f"Peak Frequency Drift RateMaximum  (Hz)          : {peak_frequency_drift_rate_maximum}"
        )
...
def _main(argsv):
    ...
    parser = argparse.ArgumentParser(
        description="Pass arguments for Bluetooth df1 Example",
