...
int main ()
{
	...
	RFmxCheckWarn(RFmxEVDO_SEMFetchCarrierMeasurementArray(instrumentHandle, "", timeout, NULL, NULL, 0, 
	...
	if( carrierMeasArraySize > 0 )
	{
		...
		{
			RFmxCheckWarn(RFmxEVDO_SEMFetchCarrierMeasurementArray(instrumentHandle, "", timeout, absoluteIntegratedPower, 
				relativeIntegratedPower, carrierMeasArraySize, NULL));
		}
		else
		{
	...
	if( actualArraySize > 0 )
	{
		...
		if( spectrum && absoluteMask && relativeMask )
		{
			RFmxCheckWarn(RFmxEVDO_SEMFetchSpectrum(instrumentHandle, "", timeout, &x0, &dx, spectrum, relativeMask,
