...
int main ()
{
    ...
    float64 carrierRelativePower = 0.00;    /*(dB) */
    ...
    RFmxCheckWarn(RFmxEVDO_CHPFetchCarrierMeasurement(instrumentHandle, "", timeout, &carrierAbsolutePower, &carrierRelativePower));

    RFmxCheckWarn(RFmxEVDO_CHPFetchSpectrum(instrumentHandle, "", timeout, NULL, NULL, NULL, 0, &actualArraySize));
    if( actualArraySize > 0 )
    {
        spectrum = (float32 *)malloc(sizeof(float32) * actualArraySize);
        if(spectrum)
        {
                RFmxCheckWarn(RFmxEVDO_CHPFetchSpectrum(instrumentHandle, "", timeout, 
