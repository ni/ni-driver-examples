...
*
*  Example Program:
*    DynamicAcquisitionHardwarePatternMatch.c
*
*  Description:
*    Configures a pattern match trigger that is used as a start,
*    reference, or pause trigger.
*
*  Pin Connection Information:
*    Connect data to desired channels
*    (default: 0-15).
*
************************************************************************/
...
int main(void)
{
   ...
   ViConstString patternToMatch = "0000 0000 0000 0000";
   ...
   /* Choose the trigger type. 0 = Start, 1 = Reference, 2 = Pause */
   ViUInt8 triggerType = 0;
   ...
   /* Configure Pattern Match Trigger */
   
   if (triggerType == 0) /* Start Trigger */
   {
      checkErr(niHSDIO_ConfigurePatternMatchStartTrigger(
               vi, channelList, patternToMatch, NIHSDIO_VAL_PATTERN_MATCHES));
   }
   else if (triggerType == 1) /* Reference Trigger */
   {
      checkErr(niHSDIO_ConfigurePatternMatchRefTrigger(
               vi, channelList, patternToMatch, NIHSDIO_VAL_PATTERN_MATCHES,
            pretriggerSamples));
   }
   else if (triggerType == 2) /* Pause Trigger */
   {
      checkErr(niHSDIO_ConfigurePatternMatchPauseTrigger(
               vi, channelList, patternToMatch, NIHSDIO_VAL_PATTERN_MATCHES));
   }
   ...
   printf("Provide pattern on configured channels.\n");
