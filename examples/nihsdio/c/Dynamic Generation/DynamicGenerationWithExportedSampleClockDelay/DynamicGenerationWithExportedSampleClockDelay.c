...
*
*  Example Program:
*    DynamicGenerationWithExportedSampleClockDelay.c
*
*  Description:
*    Exports the sample clock signal as specified (with a
*    configurable delay).
*
...
int main(void)
{
   ...
   ViInt32 exportedSampleClockMode = NIHSDIO_VAL_NONINVERTED;
   ViReal64 exportedSampleClockDelay = 0.0;
   ViInt32 dataWidth = 4;
   ...
   checkErr(niHSDIO_ConfigureSampleClock(
      ...
      /* Configure sample clock for export and specify export mode */
   ...
   checkErr(niHSDIO_SetAttributeViInt32(
            vi, VI_NULL, NIHSDIO_ATTR_EXPORTED_SAMPLE_CLOCK_MODE,
            exportedSampleClockMode));

   checkErr(niHSDIO_SetAttributeViReal64(
            vi, VI_NULL, NIHSDIO_ATTR_EXPORTED_SAMPLE_CLOCK_DELAY,
            exportedSampleClockDelay));
   ...
   /* Initiate generation of waveform and wait for completion */
   checkErr(niHSDIO_Initiate(vi));
   checkErr(niHSDIO_WaitUntilDone(vi, 10000));

   /* Un-export sample clock */
   checkErr(niHSDIO_ExportSignal(
