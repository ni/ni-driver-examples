...
/* to perform a Frequency Measurement - The functionality demonstrated by     */
/* this example is not supported by the NI-4060 and NI-4050 devices           */
/*                                                                            */
/* Functions Utilized:                                                        */
/*                                                                            */
/* niDMM_init                                                                 */
...
int main (int argc, char *argv[])
{
   ...
   ViInt32   measurementType  = NIDMM_VAL_FREQ;
   ViReal64  lowestFrequency  = 25.00;
   ViReal64  signalAmplitude  = 50.00;
   ViReal64  range            = 2500;
   ...
   checkErr(niDMM_ConfigureMeasurementDigits(vi, measurementType, lowestFrequency, resolution));

   /*- Configure Frequency Voltage Range -------------------------------*/
   checkErr(niDMM_ConfigureFrequencyVoltageRange(vi, signalAmplitude));

   /*- Read Frequency --------------------------------------------------*/
   // We use the checkWarn macro here because we can get the FREQ
   // Underrange warning from this function call
   checkWarn(niDMM_Read (vi, NIDMM_VAL_TIME_LIMIT_AUTO, &reading));
