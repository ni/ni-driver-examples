...
/*                                                                            */
/* Acquire Multiple Samples - External Trigger                                */
/*                                                                            */
/* This examples describes how to program a National Instruments Multimeter   */
/* to perform a multiple samples acquisition with external triggering.        */
/* The external trigger is an external digital pulse when the trigger source  */
...
int main (int argc, char *argv[])
{
   ...
   timeoutInMilliSeconds = timeoutInMilliSeconds + (ViInt32) (triggerDelay * 1000);
   ...
   checkErr(niDMM_ConfigureMultiPoint (vi, 1, numOfMeas, triggerSample, 0.0));
   ...
   if(triggerSource == NIDMM_VAL_SOFTWARE_TRIG)
   {
      ...
      checkErr(niDMM_FetchMultiPoint (vi, timeoutInMilliSeconds,
               numOfMeas, measurements, &numPointsRead));
  }
  else
  {
     /*- Fetch the data -----------------------------------------------*/
