...
*
* Example program:
*   ContAcqPowerSamples_IntClk_Unscaled
*
* Category:
*   Power
*
...
*   4.  Query the scaling coefficients for voltage and current.       
*   5.  Call AnalogUnscaledReader.BeginReadInt16Power to install a callback and
*       begin the asynchronous read operation.
*   6.  Inside the callback, call EndReadInt16Power to retrieve the data from
*       the read operation. Note: If the output is disabled, the data read will
*       be 0.
*   7.  Scale the voltage and current data.
*   8.  Inside the callback, query the runtime status of the task.
*   9.  Inside the callback, call BeginReadInt16Power again.
...
namespace NationalInstruments.Examples.ContAcqPowerSamples_IntClk_Unscaled
{
    /// <summary>
    ...
    public class MainForm : System.Windows.Forms.Form
    {
        ...
        private AnalogUnscaledReader analogInUnscaledReader;
        private Task myTask;
        private Task runningTask;
        private AsyncCallback analogCallback;

        private AIUnscaledPowerMeasurement[,] data;
        private double[] voltageScalingCoefficients;
        private double[] currentScalingCoefficients;
        ...
        private void InitializeComponent()
        {
            ...
            this.Text = "Continuous Acquisition of Unscaled Power Samples - Internal Clock";
        ...
        private void startButton_Click(object sender, System.EventArgs e)
        {
            if (runningTask == null)
            {
                try
                {
                    ...
                    // Query the scaling coefficients
                    voltageScalingCoefficients = myTask.AIChannels.All.PowerVoltageScalingCoefficients;
                    currentScalingCoefficients = myTask.AIChannels.All.PowerCurrentScalingCoefficients;
                    ...
                    runningTask = myTask;
                    analogInUnscaledReader = new AnalogUnscaledReader(myTask.Stream);
                    analogCallback = new AsyncCallback(AnalogInCallback);

                    // Use SynchronizeCallbacks to specify that the object 
                    // marshals callbacks across threads appropriately.
                    analogInUnscaledReader.SynchronizeCallbacks = true;
                    analogInUnscaledReader.BeginReadInt16Power(Convert.ToInt32(samplesPerChannelNumeric.Value),
        ...
        private void AnalogInCallback(IAsyncResult ar)
        {
            try
            {
                if (runningTask != null && runningTask == ar.AsyncState)
                {
                    ...
                    data = analogInUnscaledReader.EndReadInt16Power(ar);
        ...
        private double scaleData(Int16 unscaled, double[] polynomial)
        {
            // Each element of the array corresponds to a term of the equation.
            // For example, if index three of the array is 9, the fourth term
            // of the equation is 9x^3.
            double result = 0.0;
            for (int i = 0; i < polynomial.Length; ++i)
            {
                result += Math.Pow((double)unscaled, (double)i) * polynomial[i];
            }
            return result;
        }

        private void dataToDataTable(AIUnscaledPowerMeasurement[,] sourceArray, ref DataTable dataTable)
        {
            ...
            for (int channel = 0; channel < sourceArray.GetLength(0); ++channel)
            {
                ...
                for (int sample = 0; sample < sourceArray.GetLength(1); ++sample)
                {
                    ...
                    AIUnscaledPowerMeasurement data = sourceArray[channel, sample];
                    dataTable.Rows[sample][channel * 2] = scaleData(data.Voltage, voltageScalingCoefficients);
                    dataTable.Rows[sample][channel * 2 + 1] = scaleData(data.Current, currentScalingCoefficients);
                }
            }
        }
