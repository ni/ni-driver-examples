					   ...
					   LPSTR lpszCmdLine, int nCmdShow)
{
	...
	ImaqSmplHwnd = CreateWindow(ImaqSmplClassName, "LLSequence", WS_OVERLAPPEDWINDOW | WS_VISIBLE,
...
int OnSequence (void)
{
    ...
    errChk(imgCreateBufList(NUM_SEQUENCE_BUFFERS, &Bid));
	...
	for (i = 0; i < NUM_SEQUENCE_BUFFERS; i++)
	{
		...
		bufCmd = (i == (NUM_SEQUENCE_BUFFERS - 1)) ? IMG_CMD_STOP : IMG_CMD_NEXT;
		errChk(imgSetBufferElement2(Bid, i, IMG_BUFF_COMMAND, bufCmd));
		errChk(imgSetBufferElement2(Bid, i, IMG_BUFF_SKIPCOUNT, skippedBuffers[i]));
	}
    ...
    // The LPTIMECALLBACK cast is to support the 64-bit prototype which expects special 
    //     DWORD_PTR parameters. The usage is the same.
    // Set the timer
    ImaqTimerId = timeSetEvent(33, 0, (LPTIMECALLBACK)OnTimer, 0, TIME_PERIODIC);
