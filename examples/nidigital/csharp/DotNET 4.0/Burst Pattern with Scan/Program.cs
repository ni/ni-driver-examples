...
namespace NationalInstruments.Examples.NIDigital.BurstPatternWithScan
{
    using ModularInstruments.NIDigital;
    using System;

    /// <summary>
    /// Demonstrates bursting a pattern that uses the scan opcode and calling the History RAM Fetch Scan Cycle Numbers API.
    /// </summary>
    /// <remarks>
    /// Using this example:
    ...
    public sealed class Program
    {
        ...
        // Pattern settings.
        private const string PatternStartLabel = "scan_pattern";
        ...
        static void Main(string[] args)
        {
            try
            {
                ...
                using (NIDigital session = new NIDigital(ResourceName, true, true, OptionString))
                {
                    ...
                    session.PatternControl.BurstPattern("", PatternStartLabel, selectDigitalFunction, waitUntilDone, Timeout);
                    ...
                    // Fetch the scan cycle numbers
                    long[] scanCycleNumbersArray = session.HistoryRam.FetchScanCycleNumbers(site, position, samplesAvailable);

                    // Write the History RAM header to the BurstPatternWithScanResults.csv file.
                    Console.WriteLine("5. Writing the header to the BurstPatternWithScanResults.csv file.");
                    ...
                    // Write the History RAM results to the BurstPatternWithScanResults.csv file.
                    Console.WriteLine("6. Writing the History RAM results to the BurstPatternWithScanResults.csv file.");
                    UInt16 indexOfScanCycleNumbers = 0;
                    foreach (DigitalHistoryRamCycleInformation cycleInformation in cycleInformationArray)
                    {
                        string vectorNumber = cycleInformation.VectorNumber.ToString();
                        if (scanCycleNumbersArray[indexOfScanCycleNumbers] != -1)
                        {
                            vectorNumber += ":" + scanCycleNumbersArray[indexOfScanCycleNumbers].ToString();
                        }

                        string cycleInfoString = string.Format("{0},{1},{2},{3}", cycleInformation.TimeSetName, cycleInformation.PatternName,
                            vectorNumber, cycleInformation.CycleNumber.ToString());
                        ...
                        indexOfScanCycleNumbers++;
                    }
                    System.IO.File.WriteAllLines("BurstPatternWithScanResults.csv", csv);
