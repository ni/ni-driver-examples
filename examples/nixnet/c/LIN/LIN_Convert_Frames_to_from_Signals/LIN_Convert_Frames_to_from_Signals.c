...
// manual.
// This example uses hardcoded signals and frames that use the NIXNET_exampleLDF database.
...
int main(void)
{
   ...
   char *l_pSelectedSignalList = "SlaveSignal1_U16,MasterSignal1_U16";
   u32 l_NumberOfBytesForFrames = 0;
   u32 l_NumberOfBytesReturnedConv = 0;
   u8 l_Buffer[NUM_FRAMES * sizeof(nxFrameLIN_t)];
   u8 l_BufferConv[NUM_FRAMES * sizeof(nxFrameLIN_t)];
   ...
   // The frame IDs correspond to Slave1Frame1 and MasterFrame1 from the database.
   ...
   l_pFrame->Timestamp = 0;
   l_pFrame->Flags = 0;
   l_pFrame->Identifier = 2;
   l_pFrame->Type = nxFrameType_LIN_Data;
   l_pFrame->PayloadLength = 8;
   for (i = 0; i < l_pFrame->PayloadLength; ++i)
   {
      l_pFrame->Payload[i] = (u8)(i * 2);
   }
   // Print the frame we just created
   printf("%d\t", l_pFrame->Identifier);
   for (i = 0; i < l_pFrame->PayloadLength; ++i)
