/*
// This example writes signal values when a keyboard character is pressed.
...
int main(void)
{
   ...
   printf("Press any key to transmit signal value.(q to quit)\n");
   printf("Values are incremented from 0 to 10 in this example.\n");
   ...
   while ('q' != tolower(_getch()))
   {
      ...
      l_Status = nxWriteSignalSinglePoint(m_SessionRef,
         l_ValueBuffer, sizeof(l_ValueBuffer));
      if (nxSuccess == l_Status)
      {
         printf("Signals sent:\n");
         printf("Signal 1: %f\n", l_ValueBuffer[0]);
