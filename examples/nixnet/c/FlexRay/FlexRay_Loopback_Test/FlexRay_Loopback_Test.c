/*
// This example can be used to test communication between two ports.
...
int main(void)
{
   ...
   unsigned int i = 0;
   char *l_pSelectedInterface1 = "FlexRay1";
   char *l_pSelectedInterface2 = "FlexRay2";
   char *l_pSelectedDatabase = "NIXNET_example";
   char *l_pSelectedCluster = "FlexRay_Cluster";
   char *l_pSelectedInputSignalList = "FlexRayEventSignal1,FlexRayEventSignal2";
   char *l_pSelectedOutputSignalList = "FlexRayEventSignal1,FlexRayEventSignal2";
   ...
   u32 l_KeySlotIdInput = 2;
   u32 l_KeySlotIdOutput = 1;
   ...
   printf("Key Slot ID Input: %d\nl_KeySlotIdOutput: %d\n",
      l_KeySlotIdInput, l_KeySlotIdOutput);
   ...
   // Set the Key Slot of the input session
   l_Status = nxSetProperty(m_InputSessionRef,
      nxPropSession_IntfFlexRayKeySlotID, sizeof(l_KeySlotIdInput),
      &l_KeySlotIdInput);
   if (nxSuccess != l_Status)
   {
   ...
   // Set the Key Slot of the output session
   l_Status = nxSetProperty(m_OutputSessionRef,
      nxPropSession_IntfFlexRayKeySlotID, sizeof(l_KeySlotIdOutput),
      &l_KeySlotIdOutput);
   if (nxSuccess != l_Status)
   {
      DisplayErrorAndExit(l_Status, "nxSetProperty");
   ...
   while ('q' != tolower(_getch()))
   {
      ...
      // Wait 100 ms and then read back the values.
