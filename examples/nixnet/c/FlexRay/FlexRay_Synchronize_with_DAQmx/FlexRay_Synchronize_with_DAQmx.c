...
int main(void)
{
   ...
   char *l_pSelectedSignalList = "FlexRayCyclicSignal3,FlexRayCyclicSignal4";
   nxTimestamp_t l_StartTime = 0;
   f64 l_DeltaTime = 0.0;
   f64 l_ValueBuffer[NUM_SIGNALS][NUM_SAMP];
   f64 l_DaqValueBuffer[NUM_SIGNALS][NUM_SAMP];
   ...
   do
   {
      ...
      DAQmxErrChk(DAQmxReadAnalogF64(m_DaqTask, NUM_SAMP, 10,
         DAQmx_Val_GroupByChannel, (f64 *)l_DaqValueBuffer, NUM_SAMP, NULL, NULL));
      ...
      printf("\tFLEXRAY Signal 1:  [0] = %2.3f ... [50] = %2.3f, [99] = %2.3f\n",
         l_ValueBuffer[0][0], l_ValueBuffer[0][50], l_ValueBuffer[0][99]);
      printf("\tDAQ Signal 1:  [0] = %2.3f ... [50] = %2.3f, [99] = %2.3f\n",
         l_DaqValueBuffer[0][0], l_DaqValueBuffer[0][50],
         l_DaqValueBuffer[0][99]);
      printf("\n");
...
Error:
      ...
      {
         DAQmxGetExtendedErrorInfo(g_ErrBuff, 2048);
