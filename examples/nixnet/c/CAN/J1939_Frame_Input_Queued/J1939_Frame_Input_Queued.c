/*
// This example demonstrates how to receive J1939 frames using a Frame Input Queued session.
// It reads a J1939 frame in when a keyboard character is pressed except 'q'.
// This example is designed to be run with J1939 Frame Output Queued example, ensure
...
int main(void)
{
   ...
   unsigned int i = 0;
   unsigned int j = 0;
   unsigned int l_NumOfFramesReceived = 0;
   ...
   u8 l_Buffer[10000]; // Use a large buffer for J1939 queued input
   ...
   // J1939 variables
   u64 l_NodeName = 1;      // J1939 Node Name
   u32 l_InAddress = 1;     // Proposed Address
   u32 l_OutAddress = 0;    // Assigned Address by Claiming

   u16 l_PayloadLength = 0; // Received Payload Length

   // Display parameters that will be used for the example.
   printf("Interface: %s\nDatabase: %s\nFrame: %s\n", l_pSelectedInterface,
      l_pSelectedDatabase, l_pSelectedFrame);

   // Create an XNET session in FrameInQueued mode
   l_Status = nxCreateSession(l_pSelectedDatabase, l_pSelectedCluster,
      l_pSelectedFrame, l_pSelectedInterface, nxMode_FrameInQueued,
      &m_SessionRef);
   ...
   printf("\nPress any key to output the received J1939 Queued Frames."
      "(q to quit)\n");

   // Main loop
   while ('q' != tolower(_getch()))
   {
      // Read the J1939 Frame(s) Queued
      ...
      printf("There were %d frames queued.\n\n", l_NumOfFramesReceived);
   }
