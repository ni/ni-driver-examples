    ...
    public partial class MainForm : Form
    {
        ...
        void StartGeneration()
        {
            try
            {
                ...
                iData = new double[SamplesAtOnce];
                qData = new double[SamplesAtOnce];

                bReader = new BinaryReader(File.OpenRead(fileName));
                numberOfSamples = (int)bReader.BaseStream.Length;

                numberOfSamples /= 4; // I and Q waveform samples, 2 bytes each = 4 bytes per sample  -
                ...
                {
                    for (int i = 0; i < SamplesAtOnce; i++)
                    {
                        iData[i] = (double)bReader.ReadInt16() / 32768;
                        qData[i] = (double)bReader.ReadInt16() / 32768;
                    }
                    ...
                    samplesWrittenProgressBar.Value = (int)((numberOfSamples - numberRemaining) / (numberOfSamples * 0.01));
                    numberRemaining -= SamplesAtOnce;

                }
                // Read/Write the remaining numberOfSamples
                iData = new double[numberRemaining];
                qData = new double[numberRemaining];
                ...
                samplesWrittenProgressBar.Value = 100; // Done writing waveform
