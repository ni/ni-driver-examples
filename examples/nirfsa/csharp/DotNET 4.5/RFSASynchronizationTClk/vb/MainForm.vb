...
#End Region
    ...
    Private Sub ConfigureRefClock(ByVal sessionIndex As Integer)
        ...
        If sessionIndex = 0 Then
            sessions(sessionIndex).Configuration.SignalPath.LocalOscillator.LOExportEnabled = True
            loFrequency = sessions(sessionIndex).Configuration.SignalPath.LocalOscillator.LOFrequency
        Else
            _model = sessions(sessionIndex).Identity.InstrumentModel
            If String.Equals(_model, "NI PXIe-5665 (3.6GHz)") Then
                _referenceClockRate = 100000000
            Else
                _referenceClockRate = 10000000
            End If

            sessions(sessionIndex).Configuration.ReferenceClock.Rate = _referenceClockRate
            sessions(sessionIndex).Configuration.SignalPath.LocalOscillator.LOFrequency = loFrequency
        End If
