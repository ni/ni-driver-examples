namespace NationalInstruments.Examples.NIFgen.ReferenceClock
{
    using System;
    using ModularInstruments.NIFgen;

    /// <summary>
    /// Demonstrates the use of the NI-FGEN .NET API to configure the reference clock.
    ...
    /// 3. Configure the reference clock settings. If this example throws an exception when running,
    ///    the ReferenceClockSource may not be valid for the device you are using.
    ...
    public sealed class Program
    {
        ...
        private const double StandardFunctionFrequency = 1e6;

        // Reference clock settings.
        private const string ReferenceClockSource = "PXI_Clk";
        private const double ReferenceClockFrequency = 1e7;
        ...
        static void Main(string[] args)
        {
            try
            {
                ...
                using (NIFgen session = new NIFgen(ResourceName, false, true))
                {
                    ...
                    Console.WriteLine("\t - Frequency: {0} (Hz)", StandardFunctionFrequency);
                    Console.WriteLine("\t - Start Phase: {0}", StartPhase);
                    session.StandardWaveform.Configure(ChannelName, Waveform, Amplitude, DCOffset, StandardFunctionFrequency, StartPhase);

                    Console.WriteLine("4. Setting the reference clock source to {0} with a frequency of {1} Hz.", ReferenceClockSource, ReferenceClockFrequency);
                    session.Timing.ReferenceClock.Configure(ReferenceClockSource, ReferenceClockFrequency);
                    ...
                    Console.WriteLine("6. Press any key to abort generation.");
                    Console.ReadKey();
